/* Local backend.  Secrets are read from the environment/.env and are never sent
 * to the browser.  This deliberately uses only Node's standard library. */
...
function jsonBody(req) {
  return new Promise((resolve, reject) => {
    let body = '';
    req.on('data', c => { body += c; if (body.length > 2_000_000) reject(new Error('Requête trop volumineuse.')); });
    req.on('end', () => {
...
async function notifyTelegram(lead) {
  ...
  const message = [
    ...
    `Budget : ${lead.budget ?? 'Non précisé'} FCFA`,
    `Estimation : ${lead.estimate_amount ?? 'Non précisée'} FCFA`
  ].join('\n');
...
function session(req) {
  const s = sessions.get(cookie(req, 'laroche_admin'));
  if (!s || s.expiresAt < Date.now()) { if (s) sessions.delete(cookie(req, 'laroche_admin')); return null; }
  return s;
}
...
async function api(req, res, url) {
  ...
  if (p.match(/^\/api\/public\/news\/[^/]+$/) && req.method === 'GET') {
    ...
    return rows[0] ? send(res, 200, rows[0]) : fail(res, 404, 'Actualité introuvable.');
  }
  ...
  if (p === '/api/project-leads' && req.method === 'POST') {
    ...
    if (!text(contact.first_name || contact.firstName, 120) || !text(contact.last_name || contact.lastName, 120) || !emailOk(contact.email) || !text(contact.phone, 40)) return fail(res, 400, 'Coordonnées invalides.');
  ...
  if (p === '/api/plan-orders' && req.method === 'POST') {
    ...
    return send(res, 201, { ok: true, order: rows?.[0] && { id: rows[0].id, amount, currency: 'XAF', status: 'pending' }, payment: { status: 'not_configured', message: 'Le paiement Monetbil sera activé après configuration du webhook.' } });
  }
  ...
  if (p === '/api/admin/login' && req.method === 'POST') {
    ...
    if (a.until > Date.now()) return fail(res, 429, 'Trop de tentatives. Réessayez plus tard.');
    if (!emailOk(e) || !b.password || !/^\d{6}$/.test(pin)) return fail(res, 400, 'Email, mot de passe et PIN à six chiffres requis.');
    ...
    if (!admins.length || verified !== true) { a.count++; if (a.count >= 5) a.until = Date.now() + 15 * 60 * 1000; attempts.set(e, a); return fail(res, 401, 'PIN incorrect ou compte non autorisé.'); }
    attempts.delete(e); const sid = crypto.randomBytes(32).toString('hex');
    sessions.set(sid, { userId: token.user.id, expiresAt: Date.now() + 8 * 60 * 60 * 1000 });
    return send(res, 200, { ok: true }, { 'Set-Cookie': `laroche_admin=${sid}; HttpOnly; SameSite=Lax; Path=/; Max-Age=28800` });
  }
  if (p === '/api/admin/session' && req.method === 'GET') return session(req) ? send(res, 200, { authenticated: true }) : fail(res, 401, 'Session absente.');
  if (p === '/api/admin/logout' && req.method === 'POST') { sessions.delete(cookie(req, 'laroche_admin')); return send(res, 200, { ok: true }, { 'Set-Cookie': 'laroche_admin=; HttpOnly; SameSite=Lax; Path=/; Max-Age=0' }); }
